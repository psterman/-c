...
# G3 inject refresh rounds (AHK pump may consume queue before HTTP drain)
$resultPath = Join-Path $debugDir "hybrid_signoff_inject_result.json"
try {
    $wake = @{ type = "hybrid_probe_wake"; probeId = "ftb-ux-wake-$probeId" } | ConvertTo-Json -Compress
...
for ($i = 1; $i -le 5; $i++) {
    ...
    try {
        ...
        while ((Get-Date) -lt $deadline -and -not $pingOk) {
            if (Test-Path $resultPath) {
                try {
                    ...
                    if ($res.probeId -eq $roundProbe -and $res.code -eq "PING_OK") { $pingOk = $true; break }
                } catch {}
            }
        ...
        $cnt = [int]$drain.count
        if (($inj.ok -eq $true) -and (($cnt -ge 1) -or $pingOk)) { $injectOk++ } else { $injectFail++ }
    } catch {
        $injectFail++
    }
}
Add-Gate "inject_refresh" ($injectOk -ge 5 -and $injectFail -eq 0) "ok=$injectOk fail=$injectFail pumpPing=hybrid_signoff_ping"
