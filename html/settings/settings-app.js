...
function __settingsStudioTestInline(payloadJson, testId, flat) {
  ...
  return null;
    ...
    function findHotkeysSubTabByCommandId(cmdId) {
      const id = String(cmdId || "").trim();
      if (!id) return "overview";
      for (const preset of HK_VK_PRESETS) {
        if (Array.isArray(preset.commands) && preset.commands.includes(id)) return preset.id;
      }
      return "overview";
    }
...
<div class="shortcut-ref-row">
  ...
  <div><div class="shortcut-ref-label">${esc(row.label)}</div><div class="shortcut-ref-desc">${esc(row.desc)}</div></div>
    ...
    function renderVkCmdRowHtml(cmdId, cmd) {
      ...
      if (!isRec && state.hkFocusCommandId === id && Date.now() < Number(state.hkFocusPulseUntil || 0))
        rowCls += " active-focus";
  ...
  <div class="cmd-key">${combo}</div><div class="cmd-actions"></div></div>`;
    ...
    function pulseHotkeyCommandRow(cmdId) {
      const id = String(cmdId || "").trim();
      if (!id) return;
  ...
  <div class="ft-console">
    ...
    function handleHostMessage(msg) {
      ...
      if (msg.type === "initData") {
        ...
        const focusCommandId = String(p.focusCommandId || "").trim();
        const focusLabel = String(p.focusLabel || "").trim();
        ...
        if (shouldNavigate) {
          ...
          if (state.activeTab === "hotkeys") {
            state.hotkeysSubTab = findHotkeysSubTabByCommandId(focusCommandId);
          } else if (state.hotkeysSubTab === "niuma" || state.hotkeysSubTab === "qa") {
            state.hotkeysSubTab = "overview";
          }
        ...
        if (state.selectedPromptTemplateId && !ids.includes(state.selectedPromptTemplateId))
          state.selectedPromptTemplateId = "";
        render();
        if (shouldNavigate && state.activeTab === "hotkeys" && focusCommandId) {
          state.hkFocusCommandId = focusCommandId;
          state.hkFocusLabel = focusLabel || focusCommandId;
          state.hkFocusPulseUntil = Date.now() + 2800;
          setTimeout(() => pulseHotkeyCommandRow(focusCommandId), 80);
          setTimeout(() => {
            if (state.hkFocusCommandId === focusCommandId) {
              state.hkFocusCommandId = "";
              state.hkFocusLabel = "";
              state.hkFocusPulseUntil = 0;
              if (state.activeTab === "hotkeys")
                render();
            }
          }, 3000);
