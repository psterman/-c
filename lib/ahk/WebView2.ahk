...
class WebView2 {
	...
	static CreateEnvironmentAsync(options := 0, dataDir := '', edgeRuntime := '', dllPathOrFuncPtr := 'WebView2Loader.dll') {
		if !(dllPathOrFuncPtr is Integer) {
			if !FileExist(dllPathOrFuncPtr) {
				bit := (A_PtrSize * 8) 'bit'
				libDir := RegExReplace(A_LineFile, '\\[^\\]+$', '')
				libRoot := RegExMatch(libDir, '\\ahk$') ? RegExReplace(libDir, '\\ahk$', '') : libDir
				candidates := [
					libRoot '\runtime\' bit '\WebView2Loader.dll',
					libRoot '\runtime\WebView2Loader.dll',
					libRoot '\' bit '\WebView2Loader.dll',
					A_ScriptDir '\lib\runtime\' bit '\WebView2Loader.dll',
					A_ScriptDir '\lib\runtime\WebView2Loader.dll',
					A_ScriptDir '\lib\' bit '\WebView2Loader.dll',
					A_ScriptDir '\WebView2Loader.dll'
				]
				for candidate in candidates {
					if FileExist(candidate) {
						dllPathOrFuncPtr := candidate
						break
					}
				}
			}
			if !FileExist(dllPathOrFuncPtr)
				throw Error("WebView2Loader.dll 未找到。请确保 lib\runtime\64bit\WebView2Loader.dll 存在。")
