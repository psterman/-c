...
; （启动兜底 OnError 已在文件头部注册：NMER_StartupOnError）
...
$^+q:: {
    global NMER_StartupTick
    if (A_TickCount - Integer(NMER_StartupTick) < 20000) {
        try NMER_Log("startup", "reload_hotkey_ignored", "elapsed_ms=" . (A_TickCount - Integer(NMER_StartupTick)))
        return
    }
