...
_SCWV_LoadSession() {
    ...
    st := Map("uiMode", "local", "categoryKey", "academic")
    if !FileExist(path)
        return st
    try {
...
_SCWV_InferWebUiModeFromEmbedState() {
    ; 不再根据内嵌站点缓存推断 web 模式，避免本地搜索页被强行挂上联网内嵌层
    return ""
}
...
_SCWV_RejectSpuriousMinimize(allowActivate := false) {
    ...
    global g_SCWebLlm_MainWebViewLowered, g_SCWebLlm_LastBoundsKey
    g_SCWebLlm_MainWebViewLowered := false
    if _SCWV_ShouldShowWebEmbed()
        g_SCWebLlm_LastBoundsKey := ""
    if _SCWV_ShouldShowWebEmbed() && FuncExists("SearchCenterWebLlm_ApplyBounds") {
        try SearchCenterWebLlm_ApplyBounds(hwnd)
        catch {
        }
    }
}
...
_SCWV_ResolveRestoreUiMode(triggerSource := "") {
    ...
    rawUm := StrLower(Trim(String(st.Get("uiMode", "local"))))
    if (rawUm = "cli") {
        if FuncExists("CliWb_Show")
            CliWb_Show(Map())
        return "local"
    }
    if (rawUm = "web")
        return "local"
    return _SCWV_NormalizeUiMode(rawUm)
}

_SCWV_ApplyRestoredSession(triggerSource := "") {
    ts := StrLower(Trim(String(triggerSource)))
    um := _SCWV_ResolveRestoreUiMode(ts)
    if (ts != "search_hotkey" && ts != "") {
...
SCWV_IsSearchCenterForegroundOrChild() {
    ...
    if IsObject(g_SCWV_Ctrl) {
        try {
            pw := g_SCWV_Ctrl.ParentWindow
            if (pw && Integer(pw) = fg)
                return true
        } catch {
        }
    }
    if FuncExists("SearchCenterWebLlm_IsEmbedChildHwnd") {
        try {
            if SearchCenterWebLlm_IsEmbedChildHwnd(fg)
                return true
        } catch {
        }
    }
...
_SCWV_ArmWebEmbedMinimizeGuard(ms := 5000) {
    global g_SCWV_WebEmbedSuppressMinimizeUntil
    guardUntil := A_TickCount + Max(1200, Integer(ms))
    if (guardUntil > g_SCWV_WebEmbedSuppressMinimizeUntil)
        g_SCWV_WebEmbedSuppressMinimizeUntil := guardUntil
}
...
SCWV_OnGuiResize(GuiObj, MinMax, Width, Height) {
    ...
    if (MinMax = -1) {
        if _SCWV_ShouldSuppressWebModeMinimize() {
            try SCWV_Log("resize_suppress_minimize", "reason=web_mode_guard")
            catch {
            }
            SetTimer(_SCWV_RestoreFromSpuriousMinimize, -40)
            return
        }
...
SCWV_ApplyBoundsRetryTick(token := 0, reason := "", gen := 0) {
    ...
    if SCWV_ApplyBounds()
        return
    if !g_SCWV_UserMinimized && !_SCWV_ShouldShowWebEmbed() {
        try SCWV_EnsureMaximized()
        catch {
        }
    }
...
SCWV_CompositionPump(reason := "", token := 0) {
    ...
    try {
        ...
        if _SCWV_ShouldShowWebEmbed() && IsObject(g_SCWV_Gui) && FuncExists("SearchCenterWebLlm_ApplyBounds") {
            global g_SCWebLlm_MainWebViewLowered, g_SCWebLlm_LastBoundsKey
            g_SCWebLlm_MainWebViewLowered := false
            g_SCWebLlm_LastBoundsKey := ""
            try SearchCenterWebLlm_ApplyBounds(g_SCWV_Gui.Hwnd)
        ...
        hwnd := 0
        try hwnd := Integer(g_SCWV_Gui.Hwnd)
        catch {
            hwnd := 0
        }
        if hwnd && SCWV_FuncExists("WebView2_IsUsableHwnd") && WebView2_IsUsableHwnd(hwnd) {
...
SCWV_CompositionWatchdogTick(*) {
    ...
    if (g_SCWV_Visible && g_SCWV_PaintReady && g_SCWV_Gui && !g_SCWV_AwaitingReshowPaint) {
        if FuncExists("ScWebLlm_ShouldShowWebEmbed") && ScWebLlm_ShouldShowWebEmbed() {
...
SCWV_ShowHostFullscreen(retryLeft := 3) {
    ...
    if SCWV_FuncExists("Nmer_EnsureGuiMaximizedOnPopupScreen") {
        if !Nmer_IsLiveGuiWindow(g_SCWV_Gui)
            return
        try shown := !!Nmer_EnsureGuiMaximizedOnPopupScreen(g_SCWV_Gui)
        catch {
            shown := false
        }
    } else if FuncExists("Nmer_HwndFromGui") && FuncExists("Nmer_SafeShowWindow") {
        hwnd := Nmer_HwndFromGui(g_SCWV_Gui)
        if hwnd {
            ...
            hwnd := Nmer_HwndFromGui(g_SCWV_Gui)
            if hwnd {
                try shown := !!Nmer_SafeShowWindow(hwnd)
                catch {
                }
                if shown && FuncExists("Nmer_SafeMaximizeWindow")
                    try Nmer_SafeMaximizeWindow(hwnd)
                    catch {
                    }
            }
        }
        ...
        if (hwnd <= 0 || !DllCall("IsWindow", "Ptr", hwnd, "Int"))
            return
        try Nmer_MoveGuiToPopupScreen(g_SCWV_Gui, true)
        catch {
        }
    ...
    if !shown {
        if (retryLeft > 0)
            SetTimer(() => SCWV_ShowHostFullscreen(retryLeft - 1), -120)
        return
    }
    try _SCWV_ApplyHostDarkChrome(g_SCWV_Gui.Hwnd)
    catch {
...
SCWV_Hide(PersistSelection := true) {
    ...
    g_SCWV_CloseAfterReady := false
    g_SCWV_WaitingUiFinishedReveal := false
    g_SCWV_WebEmbedSuppressMinimizeUntil := 0
    SCWV_StopRevealWatchdog()
...
SCWV_WMDeactivateHideTick(*) {
    ...
    global g_SCWV_CliTerminalFocus, g_SCWebLlm_Visible, g_SCWebLlm_EmbedRequested
    if g_SCWV_CliTerminalFocus {
        try SCWV_Log("hide_skip", "reason=cli_terminal_focus")
        return
    }
    if SCWV_IsWebSearchUIMode() && (g_SCWebLlm_Visible || g_SCWebLlm_EmbedRequested) {
        try SCWV_Log("hide_skip", "reason=web_embed_active")
        return
    }
...
SCWV_WM_ACTIVATE(wParam, lParam, msg, hwnd) {
    ...
    if (hwnd = g_SCWV_Gui.Hwnd && (wParam & 0xFFFF) = 0) {
        if SCWV_IsSearchCenterForegroundOrChild() {
            try SCWV_Log("wm_activate_skip", "reason=search_center_child_foreground")
            return
        }
...
SCWV_ProcessWebMessageJson(jsonStr) {
    ...
    switch action {
        ...
        case "setCategory":
            global SearchCenterWebKeyword, g_SCWV_UiMode, SearchCenterSelectedEngines
            if msg.Has("category") {
                catKey := StrLower(Trim(String(msg["category"])))
                if (catKey = "ai") {
                    meta := Map("keyword", SearchCenterWebKeyword)
                    if IsObject(SearchCenterSelectedEngines)
                        meta["selectedEngines"] := SearchCenterSelectedEngines.Clone()
                    if FuncExists("AiWb_Show")
                        AiWb_Show(meta)
                    return
                }
                if (catKey = "cli") {
                    meta := Map("keyword", SearchCenterWebKeyword, "engine", "codex_cli")
                    if FuncExists("CliWb_Show")
                        CliWb_Show(meta)
                    return
                }
            ...
            if (ck = "cli") {
                if FuncExists("CliWb_Show")
                    CliWb_Show(Map("keyword", SearchCenterWebKeyword))
                return
            }
            if _SCWV_IsWebSearchCategoryKey(ck)
                g_SCWV_UiMode := "local"
            else if (ck = "ai")
                g_SCWV_UiMode := "local"
            if (Trim(SearchCenterWebKeyword) != "")
        ...
        case "syncSelectedEngines":
            ...
            global g_SCWebLlm_BroadcastSynced
            embedAi := g_SCWebLlm_BroadcastSynced && FuncExists("ScWebLlm_IsEmbedEngine")
                && IsObject(SearchCenterSelectedEngines) && SearchCenterSelectedEngines.Length > 0
        ...
        case "webSearch":
            ...
            return
        case "openAiWorkbench":
            meta := Map()
            if msg.Has("keyword")
                meta["keyword"] := String(msg["keyword"])
            if msg.Has("selectedEngines") && IsObject(msg["selectedEngines"])
                meta["selectedEngines"] := msg["selectedEngines"].Clone()
            if FuncExists("AiWb_Show")
                AiWb_Show(meta)
            return
        case "webLlmContentRect", "webLlmDismiss", "webLlmSelectSite", "webLlmFocusSite", "webLlmNavigate", "webLlmNav", "webLlmColumnLayout", "webLlmScroll", "webEmbedDebugRequest", "webLlmBootstrap":
            if FuncExists("SearchCenterWebLlmBridge_HandleMessage") {
                if SearchCenterWebLlmBridge_HandleMessage(msg, "search_center")
                    return
            }
            return
        case "webEmbedDebugAction":
        ...
        case "setUiMode":
            ...
            if (m = "web") {
                ...
                if (ckWeb = "" || ckWeb = "cli" || ckWeb = "ai")
                    try _SCWV_SetCategoryByKey("academic")
                    catch as _e {
                        NmerCatch(A_ThisFunc, _e)
                    }
                if (Trim(SearchCenterWebKeyword) != "")
            ...
            if (m = "web" && _SCWV_ShouldShowWebEmbed())
                SetTimer(_SCWV_DeferredWebEmbedSync, -340)
            else
                _SCWV_SyncWebEmbedForMode()
            _SCWV_PersistSession()
        ...
        case "openCliWorkbench":
            meta := Map()
            if msg.Has("keyword")
                meta["keyword"] := String(msg["keyword"])
            if msg.Has("engine")
                meta["engine"] := Trim(String(msg["engine"]))
            else if msg.Has("activeCliEngine")
                meta["engine"] := Trim(String(msg["activeCliEngine"]))
            if FuncExists("CliWb_Show")
                CliWb_Show(meta)
            return
        case "cliSend", "cliInject", "cliPaste", "cliInterrupt", "cliOpen", "cliTerminalFocus", "niuma_cli_open", "niuma_cli_restart", "niuma_cli_open_external", "niuma_cli_open_workdir":
            if FuncExists("SearchCenterCliBridge_HandleMessage") {
                if SearchCenterCliBridge_HandleMessage(msg, "search_center")
                    return
            }
            return
...
_SCWV_LoadSelectedEngines(CategoryKey) {
    ...
    if (CategoryKey = "ai" && FuncExists("ScWebLlm_NormalizeBroadcastSiteIds")) {
        embedOnly := []
        for _, ev in Engines {
            if (FuncExists("ScWebLlm_IsEmbedEngine") && ScWebLlm_IsEmbedEngine(ev))
                embedOnly.Push(ev)
        }
        if (embedOnly.Length)
            Engines := ScWebLlm_NormalizeBroadcastSiteIds(embedOnly)
        else if !Engines.Length
            Engines := ScWebLlm_DefaultOpenSiteIds()
    }
...
_SCWV_MarkAiEmbedBroadcastSynced(engines) {
    if !IsObject(engines) || !engines.Length
        return
    ...
    g_SCWebLlm_LayoutSiteIds := engines.Clone()
}
...
_SCWV_ApplySelectedEnginesFromWeb(raw) {
    ...
    for _, ev in raw {
        ...
        if (v = "")
            continue
        if (FuncExists("ScWebLlm_IsEmbedEngine") && ScWebLlm_IsEmbedEngine(v)) {
            next.Push(v)
            continue
        }
        if (!valid.Count || valid.Has(v))
            next.Push(v)
    }
    if (next.Length = 0)
        return false
    ...
    if (next.Length = SearchCenterSelectedEngines.Length) {
        ...
        if same {
            ; 与 ini 相同仍须标记已同步，否则宿主一直按 defaultOpen 5 列布局
            _SCWV_MarkAiEmbedBroadcastSynced(next)
            return false
        }
    }
    SearchCenterSelectedEngines := next
    _SCWV_SaveSelectedEngines(CategoryKey, SearchCenterSelectedEngines)
    _SCWV_MarkAiEmbedBroadcastSynced(SearchCenterSelectedEngines)
    return true
}
...
_SCWV_IsWebSearchCategoryKey(key) {
    k := StrLower(Trim(String(key)))
    if (k = "" || k = "cli")
        return false
    static webKeys := Map(
        "ai", true, "academic", true, "baidu", true, "image", true, "audio", true,
        "video", true, "book", true, "price", true, "medical", true, "cloud", true
    )
    return webKeys.Has(k)
}
...
_SCWV_DismissWebEmbed(dispose := true) {
...
_SCWV_PostWebEmbedDebugSnapshot(msg) {
    client := (msg is Map && msg.Has("client") && (msg["client"] is Map)) ? msg["client"] : Map()
...
_SCWV_HandleWebEmbedDebugAction(msg) {
    ...
    switch act {
        ...
        case "rebootstrap":
            global g_SCWV_Gui, g_SCWebLlm_EmbedBootstrapped
            if SCWV_IsWebSearchUIMode() && StrLower(Trim(String(GetSearchCenterCurrentCategoryKey()))) = "ai" {
                _SCWV_ArmWebEmbedMinimizeGuard(10000)
                _SCWV_BlockDeactivate(6000, "web_embed_debug_rebootstrap")
                if FuncExists("SearchCenterWebLlm_MarkEmbedRequested")
                    SearchCenterWebLlm_MarkEmbedRequested()
                rect := (msg is Map && msg.Has("rect") && (msg["rect"] is Map)) ? msg["rect"] : 0
                if (rect is Map) {
                ...
                if g_SCWV_Gui && FuncExists("SearchCenterWebLlm_EnsureEmbedSitesLoaded") {
                    try {
                        SearchCenterWebLlm_EnsureEmbedSitesLoaded(true, g_SCWV_Gui.Hwnd)
                        result["ok"] := true
                    } catch as e {
                ...
                ScWebLlm_Trace("debug_rebootstrap", !!result["ok"], Map("source", "debug_panel"))
            } else {
                result["error"] := "需要 web 模式且 ai 分类"
            }
        case "copy_trace":
            text := ""
            if FuncExists("SearchCenterWebLlm_BuildDebugSnapshot") {
                try {
                    ...
                    text := "=== 联网内嵌诊断 " . snap.Get("ts", A_Now) . " ===`n"
                    text .= "mode=" . snap.Get("uiMode", "") . " cat=" . snap.Get("category", "") . "`n"
                    text .= "engines=" . (IsObject(snap.Get("selectedEngines", 0)) ? snap["selectedEngines"].Length : 0) . "`n"
                    ...
                    rect := snap.Get("contentRect", Map())
                    if (rect is Map)
                        text .= "rect=" . rect.Get("width", 0) . "x" . rect.Get("height", 0) . "`n"
                    ...
                    if (logs is Map) {
                        for k, v in logs
                            text .= "`n--- " . k . " ---`n" . String(v) . "`n"
                    }
                } catch as e {
                    text := "诊断复制失败: " . e.Message
                }
            }
            if (text != "") {
                try {
...
_SCWV_OpenSearchTarget(keyword, engine) {
    ...
    if FuncExists("ScWebLlm_IsEmbedEngine") && ScWebLlm_IsEmbedEngine(eng) {
        ...
        if FuncExists("AiWb_Show") {
            try {
                AiWb_Show(meta)
                if FuncExists("SearchCenterWebLlm_NavigateEngine")
                    return SearchCenterWebLlm_NavigateEngine(eng, kw)
                return true
            } catch as _e {
                NmerCatch(A_ThisFunc, _e)
            }
    ...
    if (SearchCenterSelectedEngines.Length > 0) {
        if FuncExists("ScWebLlm_EnabledSites") {
            ...
            for site in ScWebLlm_EnabledSites() {
                sid := site["id"]
                if (ArrayContainsValue(SearchCenterSelectedEngines, sid) <= 0) {
                    SearchCenterSelectedEngines.Push(sid)
                    changed := true
                }
            }
            if changed
...
_SCWV_SendToCLI(prompt, engine := "") {
    if FuncExists("ScCli_SendToCLI")
        ScCli_SendToCLI(prompt, engine)
}

_SCWV_InjectPromptToTtyd(prompt, engine := "") {
    if FuncExists("ScCli_InjectPromptToTtyd")
        return ScCli_InjectPromptToTtyd(prompt, engine)
    return false
}

_SCWV_PasteToTtyd(engine := "") {
    if FuncExists("ScCli_PasteToTtyd")
        return ScCli_PasteToTtyd(engine)
    return false
}

_SCWV_InterruptTtyd(engine := "") {
    if FuncExists("ScCli_InterruptTtyd")
        return ScCli_InterruptTtyd(engine)
    return false
}

_SCWV_FocusTtydForEngine(engine := "") {
    if FuncExists("ScCli_FocusTtydForEngine")
        return ScCli_FocusTtydForEngine(engine)
    return false
}

_SCWV_OpenCliWorkDir(engine := "") {
    if FuncExists("ScCli_OpenCliWorkDir")
        return ScCli_OpenCliWorkDir(engine)
    return false
}
